 ...
 * 封装微信音乐播放API
 *
 * https://github.com/zengdongbao/weixinapp/tree/master/utils/player
 */
...
event.defaults({
    ...
    play:function(){
        ...
        COUNT_PAUSE_TRIGGER = 0;
        cb.apply(null,arguments);
        // console.log('default event play')
    },
    //播放暂停
    pause:function(){
        data.status = PAUSED;
        ...
        COUNT_PAUSE_TRIGGER++;
        cb.apply(null,arguments);
        // console.log('default event pause')
    },
    //停止播放
    ...
    stop:function(){
        ...
        COUNT_PAUSE_TRIGGER = 0;
        cb.apply(null,arguments);
        // console.log('default event stop')
    },
    //播放结束
    end:function(){
        // play(data.mode === LOOPONE ? list.current() : list.next()); // edited by 骏 20170310: 不需要循环，请在事件绑定
        cb.apply(null,arguments);
        // console.log('default event end')
    },
    //播放进度
    ...
    preplay:function(){
        cb.apply(null,arguments);
        // console.log('default event preplay')
    }
});
...
function play(single,options){
    ...
    event.trigger('preplay',function(single){
        ...
        try{
            ...
            // console.log('I am in preplay, and will play')
        }catch (e){
            console.log(e)
            throw e;
        }
        // console.log('I am in preplay')
    },single);
...
function pause(){
    ...
    wx.pauseBackgroundAudio({
        success: function(){
            ...
            // edited by 骏 in 20170315 安卓存在不触发事件情况，改在别处调起trigger
            var res = wx.getSystemInfoSync();
            if(res && !COUNT_PAUSE_TRIGGER && /android/i.test(res.system)){
                console.log('isAndroid & will trigger pause');
...
//指定播放
function invoke(index,option){
    stop();
    play(list.get(index),option);
}


//继续播放
function resume(option){
    play(data.audio,option);
}

//开始播放
function start(option){
    play(list.first(),option);
}
...
function next(option){
    play(list.next(),option);
}
//上一首
function prev(option){
    play(list.prev(),option);
}
...
Player.prototype = {
    ...
    // edited by 骏 20170310: 允许唤醒timeupadate及其timer运作
    // edited by 骏 20170315: 暂时不需要使用该方法，改用在页面通过标记判断是否使用 timeupdate 方法
    resumeTimeupdate: function(){
        if(event.isHold('timeupdate')){
            event.resume('timeupdate');
            bindTimeupdate();
        }
